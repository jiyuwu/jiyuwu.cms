...
{
    public partial class Base_MenuService
    {
        /// <summary>
        /// 菜单静态化处理，每次获取菜单时先比较菜单是否发生变化，如果发生变化从数据库重新获取，否则直接获取_menus菜单
        /// </summary>
        private static List<Base_Menu> _menus { get; set; }
            ...
            lock (_menuObj)
            {
                ...
                _menus.ForEach(x =>
                {
                    ...
                    if (!string.IsNullOrEmpty(x.Auth) && x.Auth.Length > 10)
                    {
                        try
                        {
                            x.Actions = x.Auth.DeserializeObject<List<Base_Actions>>();
                        }
                        catch { }
                    }
                    if (x.Actions == null) x.Actions = new List<Base_Actions>();
                });
        ...
        /// </summary>
        /// <returns></returns>
        public List<Base_Menu> GetCurrentMenuList()
        {
        ...
        public List<Base_Menu> GetUserMenuList(int[] roleId)
        {
        ...
        /// <returns></returns>
        public async Task<WebResponseContent> Save(Base_Menu menu)
        {
            ...
            try
            {
                ...
                if (menu.TableName != "/" && menu.TableName != ".")
                {
                    ...
                    Base_Menu sysMenu = await repository.FindAsyncFirst(x => x.TableName == menu.TableName);
                    if (sysMenu != null)
                    {
                        sysMenu.MenuType ??= 0;
        ...
        public async Task<WebResponseContent> DelMenu(int menuId)
        {
            ...
            repository.Delete(new Base_Menu()
            {
                Menu_Id = menuId
            }, true);
            CacheContext.Add(_menuCacheKey, DateTime.Now.ToString("yyyyMMddHHMMssfff"));
