...
{
    public partial class Base_RoleService
    {
        private WebResponseContent _responseContent = new WebResponseContent();
        public override PageGridData<Base_Role> GetPageData(PageDataOptions pageData)
        {
            //角色Id=1默认为超级管理员角色，界面上不显示此角色
            QueryRelativeExpression = (ISugarQueryable<Base_Role> queryable) =>
            {
        ...
        public async Task<WebResponseContent> GetUserTreePermission(int[] roleIds)
        {
            ...
            //权限用户权限查询所有的菜单信息
            List<Base_Menu> menus = await Task.Run(() => Base_MenuService.Instance.GetUserMenuList(roleIds));
        ...
        private List<Base_Actions> GetActions(int menuId, List<Base_Actions> menuActions, List<Permissions> permissions, int[] roleIds)
        {
        ...
        public async Task<WebResponseContent> SavePermission(List<UserPermissions> userPermissions, int roleId)
        {
            ...
            try
            {
                ...
                //被分配角色的权限
                List<Base_RoleAuth> roleAuths = await repository.FindAsync<Base_RoleAuth>(x => x.Role_Id == roleId);
                List<Base_RoleAuth> updateAuths = new List<Base_RoleAuth>();
                foreach (UserPermissions x in userPermissions)
                {
                    ...
                    {
                        updateAuths.Add(new Base_RoleAuth()
                        {
                            Role_Id = roleId,
                            Menu_Id = x.Id,
                ...
                .Select(s => s.Auth_Id)
                .ToArray();
                List<Base_RoleAuth> delAuths = roleAuths.Where(x => x.AuthValue != "" && !authIds.Contains(x.Auth_Id)).ToList();
                delAuths.ForEach(x =>
                {
                    x.AuthValue = "";
                    x.AuthMenuData = "";
        ...
        {
            AddOnExecuting = (Base_Role role, object obj) =>
            {
        ...
        private WebResponseContent ValidateRoleName(Base_Role role, Expression<Func<Base_Role, bool>> predicate)
        {
        ...
        {
            UpdateOnExecuting = (Base_Role role, object obj1, object obj2, List<object> obj3) =>
            {
                if (role.Role_Id == 1)
                {
                    role.ParentId = 0;
