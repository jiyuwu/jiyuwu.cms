    ...
    public class UserContext
    {
        ...
        private List<Permissions> ActionToArray(List<Permissions> permissions)
        {
            permissions.ForEach(x =>
            {
                try
                {
                    var menuAuthArr = x.MenuAuth.DeserializeObject<List<Base_Actions>>();
        ...
        private List<Permissions> MenuActionToArray(List<Permissions> permissions)
        {
            permissions.ForEach(x =>
            {
                try
                {
                    x.UserAuthArr = string.IsNullOrEmpty(x.UserAuth)
                    ? new string[0]
                    : x.UserAuth.DeserializeObject<List<Base_Actions>>().Select(s => s.Value).ToArray();
                }
                catch { }
                finally
                {
                ...
                && currnetVeriosn == cacheService.Get(authKey)))
            {
                ...
                lock (objId)
                {
                    if (!((userAuthVersion.TryGetValue(userId, out currnetVeriosn)
                        && currnetVeriosn == cacheService.Get(authKey))))
                    {
                        var authQuery = DbManger.BaseDbContext.Queryable<Base_UserAuth>()
                                   ...
                                   .Select(s => new
                                   {
                                       ...
                                       s.AuthUserIds,
                                       s.MenuId,
                                       TableName = SqlSugar.SqlFunc.Subqueryable<Base_Menu>().Where(x => x.Menu_Id == s.MenuId).Select(x => x.TableName)
                                   });
        ...
        /// 获取用户所有数据库
        /// </summary>
        public IEnumerable<Base_DbService> UserDbService
        {
            get
            {
                if (IsSuperAdmin)
