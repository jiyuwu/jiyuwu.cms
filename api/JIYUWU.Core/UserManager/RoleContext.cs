...
namespace JIYUWU.Core.UserManager
{
    public static class RoleContext
    {
        ...
        {
            RoleFieldsList = DbServerProvider.DbContext.Set<Base_RoleFields>()
        ...
        public static void UpdateRoleFields(Base_RoleFields roleFields)
        {
            lock (_obj_fields)
            {
        ...
        public static ISugarQueryable<int> GetCurrentAllChildUser()
        {
            ...
            return DbServerProvider.DbContext
                  .Set<Base_User>()
                  .Where(u => SqlFunc.Subqueryable<Base_UserRole>().Where(x => roles.Contains(x.RoleId) && x.Enable == 1 && x.UserId == u.User_Id).Any()).Select(s => s.User_Id);
        }
