...
namespace JIYUWU.Core.WorkFlow
{
    public class WorkFlowContainer
    {
            ...
            Task.Run(() =>
             {
                 try
                 {
                     List<Base_WorkFlow> list = null;
                     List<string> tables = _container.Select(s => s.Key).ToList();
                     list = DbManger.BaseDbContext.Set<Base_WorkFlow>()
                     .Includes(x => x.Base_WorkFlowStep)
        ...
        private WorkFlowContainer InitOptions(Type type, List<Base_WorkFlow> list)
        {
            ...
            foreach (var workFlow in list)
            {
                try
                {
                    ...
                    obj.Invoke(this, new object[] { workFlow, workFlow.Base_WorkFlowStep, true });
                }
                catch (Exception e)
                {
        ...
        public WebResponseContent AddTable(Base_WorkFlow workFlow, List<Base_WorkFlowStep> flowSteps, bool showError = true)
        {
        ...
        public static WebResponseContent Add<T>(Base_WorkFlow workFlow, List<Base_WorkFlowStep> flowSteps, bool showError = true) where T : class
        {
            WebResponseContent webResponse = new WebResponseContent();
            lock (_wk_object)
            {
                ...
                if (success)
                {
                    if (options.Base_WorkFlowStep == null)
                    {
                        options.Base_WorkFlowStep = flowSteps;
                    }
