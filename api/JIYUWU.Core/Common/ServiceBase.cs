...
namespace JIYUWU.Core.Common
{
            ...
            where TRepository : IRepository<T>
    {
        ...
        public virtual WebResponseContent Import(List<Microsoft.AspNetCore.Http.IFormFile> files)
        {
            ...
            {
                //生成雪花id
                var idWorker = new Common.IdWorker();
                foreach (var item in list)
                {
        ...
        public virtual WebResponseContent Add(SaveModel saveDataModel)
        {
            ...
            {
                saveDataModel.MainData.Add(keyPro.Name, new Common.IdWorker().NextId());
            }
            else if (keyPro.PropertyType == typeof(string))
            {
                if (keyProVal.IsNullOrEmpty())
                    saveDataModel.MainData.Add(keyPro.Name, new Common.IdWorker().NextId().ToString());  //adams 2023.4.27 雪花算法
            }
            else
            {
            ...
            Response = repository.DbContextBeginTransaction(() =>
            {
                ...
                if (list != null && list.Count > 0)
                {
                    ...
                    list.ForEach(x =>
                    {
                        ...
                        if (logicDelProperty != null)
                        {
                            logicDelProperty.SetValue(x, ((int)DelStatus.正常).ChangeType(logicDelEntityProperty.PropertyType));
                        }
                        //设置用户默认值
        ...
        private void SetEntityValue(T entity)
        {
            ...
            {
                var idWork = new Common.IdWorker();
                //adams 2023.11.8
        ...
        public virtual WebResponseContent RestartWorkFlowAudit(object id, string msg, FlowWriteState flowWriteState = FlowWriteState.重新开始)
        {
            ...
            Response = repository.DbContextBeginTransaction(() =>
            {
                return WorkFlowManager.Audit<T>(repository.BaseDbContext, entity, WorkFlow.AuditStatus.审核未通过, msg, autditProperty: GetAuditProperty(), flowWriteState: flowWriteState, workFlowTableName: WorkFlowTableName);
            });
            return Response;
        }
