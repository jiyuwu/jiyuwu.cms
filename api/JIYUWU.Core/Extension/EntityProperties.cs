...
{
    private static List<Base_CodeRule> codeRules = null;
    private static object ruleObject = new object();

    public static string GetCode<T>(this T entity) where T : class
    {
        Init();
    ...
    public static void Init()
    {
        if (codeRules == null)
        {
            lock (ruleObject)
            {
                if (codeRules == null)
                {
                    codeRules = DbServerProvider.DbContext.Set<Base_CodeRule>().ToList();//.FilterTenancy()
                }
            }
        }
    }
     ...
     ) where T : class
    {
        ...
        if (dateFormat != null)
        {
            rule = $"{preCode}{concatenationSymbol}{DateTime.Now.ToString("yyyyMMdd")}";
        }
        else
        {
            rule = preCode;
        ...
        {
            code = $"{rule}{concatenationSymbol}{(number + 1).ToString("D" + len)}";
